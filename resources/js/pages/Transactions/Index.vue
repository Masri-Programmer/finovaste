...
const getTypeLabel = (type: string) => {
    switch (type) {
        case 'buy_now': return 'transactions.types.purchase';
        case 'auction_buy_now': return 'transactions.types.auction_buy_now';
        case 'investment': return 'transactions.types.investment';
        case 'donation': return 'transactions.types.donation';
        case 'auction': return 'transactions.types.auction';
        default: return type;
    }
};
            ...
            <Table>
                <TableHeader>
                    <TableRow>
                        ...
                        <TableHead>{{ $t('transactions.columns.item') }}</TableHead>
                        <TableHead>{{ $t('transactions.columns.amount') }}</TableHead>
                        <TableHead>{{ $t('transactions.columns.status') }}</TableHead>
                        <TableHead class="text-right">{{ $t('transactions.columns.action') }}</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    <TableRow v-for="transaction in transactions.data" :key="transaction.id">
                        ...
                        <TableCell>
                            <span v-if="transaction.payable && transaction.payable.listing" class="truncate max-w-[200px] block">
                                {{ transaction.payable.listing.title[$page.props.locale] }}
                            </span>
                            <span v-else class="text-muted-foreground italic">{{ $t('transactions.item_unavailable') }}</span>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell class="text-right">
                                <Link v-if="transaction.payable && transaction.payable.listing" :href="show.url(transaction.payable.listing.id)">
                                <Button variant="ghost" size="sm">{{ $t('transactions.actions.view') }}</Button>
                                </Link>
                        </TableCell>
                    </TableRow>
